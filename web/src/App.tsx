...
function App() {
  ...
  return <div className="app">
    <header><h1>イヌ式市場|半自動化UI</h1><p className="subtitle">工程チャット固定運用・手動コピペ支援版</p><div className="status"><span>Mode: {health?.mode ?? 'manual copy-paste support'}</span><span>スマホURL: http://{health?.localIp || '192.168.x.x'}:5173</span></div><nav><button onClick={()=>setView('home')}>ホーム</button><button onClick={()=>setView('settings')}>設定</button><button onClick={()=>setView('templates')}>プロンプト管理</button><button onClick={()=>setView('logs')}>ログ</button></nav></header>
    {toast && <div className="alert" onClick={()=>setToast('')}>{toast}</div>}
    {view==='settings' && settings && <SettingsView settings={settings} setSettings={setSettings} notify={notify} copyText={copyText} />}
    {view==='templates' && templates && <TemplatesView templates={templates} setTemplates={setTemplates} notify={notify} copyText={copyText} />}
...
function BatchDetail({ batch, setBatch, refresh, copyText, notify, openStepChat }:{ batch:Batch; setBatch:(b:Batch)=>void; refresh:(id?:string)=>Promise<void>; copyText:(t:string,l?:string)=>void; notify:(m:string)=>void; openStepChat:(s:StepKey)=>void }) {
  ...
  return <section className="card"><h2>バッチ詳細</h2><div className="progress"><span>作成: {new Date(batch.createdAt).toLocaleString()}</span><span>コマンド: {batch.command}</span><span>{batch.stocks.length}銘柄</span><span>Status: {batch.status}</span></div><h3>一括コピー支援</h3><div className="actions"><select value={bulkStep} onChange={(e)=>setBulkStep(e.target.value as StepKey)}>{stepKeys.filter((s)=>s!=='step0').map((s)=><option value={s} key={s}>工程{s.replace('step','')}：{stepNames[s]}（入力: {inputSourceByStep[s]}）</option>)}</select><button onClick={()=>openStepChat(bulkStep)}>工程チャットを開く</button></div>{pending.length ? <div className="bulk-list">{pending.slice(0, 5).map((stock)=><div className="bulk" key={stock.stockId}><strong>{stock.name} {stock.code}</strong><CopyControls batch={batch} stock={stock} step={bulkStep} setBatch={setBatch} refresh={refresh} copyText={copyText} notify={notify} openStepChat={openStepChat} compact /></div>)}</div> : <p className="hint">この工程は全件完了しています。</p>}<h3>銘柄カード</h3>{batch.stocks.map((stock)=><div key={stock.stockId}><StockCard batch={batch} stock={stock} setBatch={setBatch} refresh={refresh} copyText={copyText} notify={notify} openStepChat={openStepChat}/></div>)}</section>;
}
...
function SettingsView({ settings, setSettings, notify, copyText }:{ settings:Settings; setSettings:(s:Settings)=>void; notify:(m:string)=>void; copyText:(t:string,l?:string)=>void }) {
...
function TemplatesView({ templates, setTemplates, notify, copyText }:{ templates:Templates; setTemplates:(t:Templates)=>void; notify:(m:string)=>void; copyText:(t:string,l?:string)=>void }) {
  ...
  return <section className="card"><h2>プロンプト管理</h2><p className="hint">このプロンプトを工程チャット冒頭に貼ってください。通常作業ではプロンプト全文を毎回貼らず、工程チャットを開いて入力文だけをコピーします。</p>{(Object.keys(draft) as Array<keyof Templates>).map((k)=><div key={k} className="settings-row"><label>{templateLabels[k]}</label><textarea className="prompt" value={draft[k]} onChange={(e)=>setDraft({...draft,[k]:e.target.value})}/><div className="actions"><button onClick={()=>copyText(draft[k], `${templateLabels[k]}をコピーしました`)}>このプロンプトをコピー</button><button onClick={()=>copyText(`このチャットでは以下のプロンプトに従い、以後ユーザーが入力コードまたは本文を送信したら、このプロンプトのルールに従って出力してください。\n\n${draft[k]}`, '初期化文をコピーしました')}>初期化文をコピー</button><button onClick={()=>copyText(`${draft[k]}\n\nUI貼り戻し用出力ルール：完成した出力だけを返し、余計な説明を追加しないでください。`, '出力ルール付きプロンプトをコピーしました')}>UI貼り戻し用出力ルール付きでコピー</button></div></div>)}<div className="actions"><button onClick={save}>プロンプトを保存</button><button onClick={reset}>初期値/保存済みに戻す</button></div><ExportImport notify={notify}/></section>;
}

function ExportImport({ notify }:{ notify:(m:string)=>void }){ const [text,setText]=useState(''); const [mode,setMode]=useState<'append'|'overwrite'>('append'); async function exp(kind:string){setText(JSON.stringify(await api.export(kind),null,2)); notify('エクスポートJSONを作成しました。');} async function imp(){await api.import({ ...JSON.parse(text), importMode: mode }); notify('インポートしました。画面を更新してください。');} return <div><h3>JSONエクスポート/インポート</h3><p className="hint">全体JSONには batches / settings / promptTemplates(templates) / favoriteCommands / project/chat URLs が含まれます。同じbatchIdの扱いは、上書きまたは追加モードを選んでください。</p><div className="grid"><button onClick={()=>exp('all')}>全体JSONエクスポート</button><button onClick={()=>exp('batches')}>バッチJSON</button><button onClick={()=>exp('settings')}>設定JSON</button><button onClick={()=>exp('templates')}>プロンプトJSON</button></div><label>インポート方式</label><select value={mode} onChange={(e)=>setMode(e.target.value as 'append'|'overwrite')}><option value="append">既存データに追加（同じbatchIdは現在の実装ではJSON内容を反映）</option><option value="overwrite">既存データを上書き</option></select><textarea value={text} onChange={(e)=>setText(e.target.value)} placeholder="インポートするJSON、またはエクスポート結果"/><button onClick={imp}>JSONインポート</button></div>}
